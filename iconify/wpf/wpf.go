...
func IconFromName(name string) g.Node {
	...
	case "repeat":
		return Repeat()
	case "restrictionShield":
		return RestrictionShield()
	case "retroTv":
		return RetroTv()
	case "rewind":
		return Rewind()
	...
	case "todoList":
		return TodoList()
	case "twoFSwipeDown":
		return TwoFSwipeDown()
	case "twoFSwipeRight":
		return TwoFSwipeRight()
	case "unlock":
		return Unlock()
	case "unlockTwo":
		return UnlockTwo()
...
func AddImage(children ...g.Node) g.Node {
	return s.SVG(
		...
		g.Attr("height", "none"),
		g.Attr("style", "width: 26px; height: 26px; display: block;"),
