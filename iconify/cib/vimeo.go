package cib

import (
	g "github.com/maragudk/gomponents"
	s "github.com/maragudk/gomponents/svg"
)

func Vimeo(children ...g.Node) g.Node {
	return s.SVG(
		g.Attr("viewbox", "0 0 32 32"),
		g.Raw(`<path fill="currentColor" d="M27.2 2H4.8A2.803 2.803 0 0 0 2 4.8v22.4C2 28.744 3.256 30 4.8 30h22.4c1.544 0 2.8-1.256 2.8-2.8V4.8C30 3.256 28.744 2 27.2 2zm-1.637 9.3c-.087 1.969-1.462 4.669-4.125 8.087c-2.75 3.575-5.081 5.363-6.981 5.363c-1.181 0-2.175-1.087-2.994-3.269c-1.594-5.831-2.275-9.25-3.587-9.25c-.15 0-.681.319-1.587.95l-.95-1.225c2.331-2.05 4.556-4.325 5.95-4.45c1.575-.15 2.544.925 2.906 3.231c1.294 8.2 1.869 9.438 4.225 5.725c.844-1.337 1.3-2.356 1.363-3.056c.219-2.075-1.619-1.931-2.863-1.4c.994-3.256 2.894-4.838 5.7-4.75c2.081.056 3.063 1.406 2.944 4.044z"/>`),
		g.Group(children),
	)
}