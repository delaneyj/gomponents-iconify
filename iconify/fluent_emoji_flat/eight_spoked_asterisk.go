package fluent_emoji_flat

import (
	g "github.com/maragudk/gomponents"
	s "github.com/maragudk/gomponents/svg"
)

func EightSpokedAsterisk(children ...g.Node) g.Node {
	return s.SVG(
		g.Attr("viewbox", "0 0 32 32"),
		g.Raw(`<g fill="none"><path fill="#00D26A" d="M2 6a4 4 0 0 1 4-4h20a4 4 0 0 1 4 4v20a4 4 0 0 1-4 4H6a4 4 0 0 1-4-4V6Z"/><path fill="#fff" d="M16 6a1 1 0 0 0-1 1v6.586l-4.657-4.657a1 1 0 0 0-1.414 1.414L13.586 15H7a1 1 0 1 0 0 2h6.586l-4.657 4.657a1 1 0 0 0 1.414 1.414L15 18.414V25a1 1 0 1 0 2 0v-6.586l4.657 4.657a1 1 0 0 0 1.414-1.414L18.414 17H25a1 1 0 1 0 0-2h-6.586l4.657-4.657a1 1 0 0 0-1.414-1.414L17 13.586V7a1 1 0 0 0-1-1Z"/></g>`),
		g.Group(children),
	)
}